...
def split_dataset_class(train_dataset): 
    dataset1 = [] 
    dataset1_targets = [] 
    dataset2 = [] 
    dataset2_targets = [] 
    for i in range(0, len(train_dataset)):
        ...
        if len(dataset2) == 30000:
            ...
            dataset1_targets.append(train_dataset[i][1])
            continue 
        if train_dataset[i][1] in [1, 4, 7, 9, 10]: 
            dataset1.append(np.array(train_dataset[i][0]))
            dataset1_targets.append(train_dataset[i][1])
        else:
            dataset2.append(np.array(train_dataset[i][0]))
...
def split_dataset_random(train_dataset): 
    dataset_train1, dataset_train2 = torch.utils.data.random_split(train_dataset, [30000, 30000])
    dataset1 = [] 
    dataset1_targets = [] 
    dataset2 = [] 
    dataset2_targets = [] 

    for val,target in dataset_train1 : 
        dataset1.append(np.array(val))
        dataset1_targets.append(target)
    for val,target in dataset_train2: 
        dataset2.append(np.array(val))
        dataset2_targets.append(target)
...
def main(): 
    ...
    for i in range(0, len(dataset_train)):
        ...
        if dataset_train[i][1] in [1, 4, 7, 9, 10]: 
            ...
            dataset2_targets.append(dataset_train[i][1])
    """
    dataset1, dataset1_targets, dataset2, dataset2_targets = split_dataset_variance(dataset_train) 
